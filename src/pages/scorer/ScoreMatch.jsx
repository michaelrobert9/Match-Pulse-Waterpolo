...
export default function ScoreMatch() {
  ...
  const [pasteFor,     setPasteFor]     = useState(null) // side being late-pasted (→ competition squad)
  const [pasteFixtureFor, setPasteFixtureFor] = useState(null) // side being pasted → THIS fixture's line-up (leagues/standalone, §3)
  ...
  // confirmation tap before it freezes at start — the moment a human is looking
  // at the real team. Directly-pasted / manual line-ups skip this: someone
  // already confirmed the grid for this specific fixture.
  const startNeedsLineupConfirm = () =>
    ['home', 'away'].some(s => isInheritedLineup(match, compType) && (teamSheets?.[s]?.squad?.length ?? 0) > 0)

  const handleStart = () => {
    unlockAudio()
    if (startNeedsLineupConfirm() && !lineupConfirmForStart) {
      setLineupSide(['home', 'away'].find(s => (teamSheets?.[s]?.squad?.length ?? 0) > 0) || 'home')
  ...
  // standalone fixture through the paste-and-grid screen only, which writes the
  // whole side's line-up at once (setPasteFixtureFor → TeamSheetEditor).
  ...
  return (
    <div className={`max-w-2xl mx-auto overflow-hidden ${t.root} flex flex-col transition-colors`} style={{ height: '100dvh' }}>
        ...
        <Sheet t={t} title="Match Lineup" onClose={() => setLineupOpen(false)}>
          ...
          {lineupConfirmForStart && (
            <div className="mb-3">
              ...
              <p className={`text-[11px] ${t.muted} mt-1.5 text-center`}>
          ...
          {!(isInheritedLineup(match, compType) && (teamSheets?.[lineupSide]?.squad?.length ?? 0) > 0) && (
            <button
              onClick={() => { setLineupError(''); setPasteFixtureFor(lineupSide) }}
              className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm uppercase tracking-wider rounded-xl py-3">
              Paste team sheet
            </button>
          )}
      ...
      {/* League / standalone fixture (§2/§3): the paste-and-grid is the only way
          to add players. It writes the confirmed grid straight to THIS fixture's
          line-up for the side — no competition squad, no inheritance. */}
      {pasteFixtureFor && (
        <TeamSheetEditor
          competitionId={match.competitionId ?? null}
          matchId={match.id}
          side={pasteFixtureFor}
          team={{
            id: pasteFixtureFor === 'home' ? match.homeTeamId : match.awayTeamId,
            displayName: teamName(pasteFixtureFor),
            orgName: pasteFixtureFor === 'home' ? (match.homeOrgName ?? null) : (match.awayOrgName ?? null),
          }}
          onClose={() => setPasteFixtureFor(null)}
          onSaved={() => setPasteFixtureFor(null)}
        />
      )}
