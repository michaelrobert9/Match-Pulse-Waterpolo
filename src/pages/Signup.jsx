...
// Water polo positions — from the sport profile module (single source).
...
export default function Signup() {
  ...
  async function handleStep1(e) {
    ...
    try {
      ...
      if (err.code === 'auth/email-already-in-use') {
        navigate('/login', { state: { from: '/', prefillEmail: email,
          notice: 'You already have a MatchPulse account with this email — sign in instead.' } })
        return
      }
      const msg = {
  ...
  async function saveProfile() {
    ...
    // Water polo position + WPSA number → waterpoloProfiles/{uid} (brief §3),
    // never the shared central users/{uid} doc.
    await saveSportProfile(createdUid, {
      position:   role === 'player' ? position : '',
      wpsaNumber: sahaNumber.trim(),
    }).catch(() => {})
  }
