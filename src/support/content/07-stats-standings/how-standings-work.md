...
The standings table ranks the teams in a competition by their results. It updates itself as matches are finalised, so it's always current.
...
Teams earn points from their results, and the table orders them accordingly, typically points first, then the usual tie-breakers your competition uses. As more matches finalise, the table fills out and settles towards the final standings.

## When the season ends

Once every match is finalised, the table is settled and reflects the final standings. Finalising the competition locks this in. See [Finalise a competition](../03-competitions/finalise-a-competition.md).

## If the table looks off

Because the table is computed from results, a wrong-looking table almost always traces back to a wrong or missing result. Fix the match, and the table corrects itself. See [Why a stat looks wrong](why-a-stat-looks-wrong.md).
