...
function titleCaseWord(word, isFirst) {
  ...
  if (lower.includes('-')) {
    return lower.split('-').map(part => part ? part[0].toUpperCase() + part.slice(1) : part).join('-')
  }
  return lower ? lower[0].toUpperCase() + lower.slice(1) : lower
}

// Normalise an ALL CAPS name to title case, preserving McDonald, van der
// Merwe, O'Brien, du Plessis. Mixed-case input is returned untouched — a
// correctly-typed "van der Merwe" must never be mangled.
